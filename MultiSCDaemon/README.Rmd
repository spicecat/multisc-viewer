...
msc_plumb(port = 8000, host = "0.0.0.0")
```
...
|----------|---------|-------------|
| `PLUMBER_PORT` | 8000 | HTTP port (alternative to `port` argument) |
...
Once running: `http://localhost:8000/` serves the API root. Interactive docs (if enabled) usually at `http://localhost:8000/__docs__/`.

## Function reference

`msc_plumb(port = plumber::get_option_or_env("plumber.port", 8000), ...)`

Arguments:

* `port` – numeric port to bind (defaults to env `PLUMBER_PORT` or 8000)
...
## Not run:
# httr::GET("http://localhost:8000/datasets")
```

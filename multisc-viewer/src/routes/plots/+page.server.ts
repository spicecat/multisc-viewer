...
import { getDatasets, getGenesRows, getPublications } from "$lib/server/data";
import { getPlotId, plots } from "$lib/server/plots";
import type { DatasetsMap, PlotsParams, Publication } from "$lib/types/daemon";
import type { PageServerLoad } from "./$types";

export const load: PageServerLoad = async ({ url }) => {
	let publication: Publication | undefined, datasets: DatasetsMap | undefined;
		...
		ds.map((d) => [
			...
			Object.fromEntries(
				gene.map((g) => [
					g,
					Object.fromEntries(
						pt.map((p) => [p, getPlotId(d, g, p, groupBy, splitBy)]),
					),
				]),
			),
		]),

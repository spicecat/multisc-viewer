...
export class Daemon {
	...
	public exec(executor: (proc: ChildProcessWithoutNullStreams, opid: string) => void): Promise<void> {
	...
	public async render(ds: string, gene: string, groupBy: string, splitBy: string): Promise<string> {
		if (!this._datasets.some((dataset) => dataset.ds === ds)) throw new Error(`Attempt to render unloaded dataset ${ds}`);
	...
	public async unload(ds: string): Promise<void> {
		if (!this._datasets.some((dataset) => dataset.ds === ds)) throw new Error(`Attempt to unload unloaded dataset ${ds}`);

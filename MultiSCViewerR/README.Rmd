...
Backend service to serve data and plots for [MultiSC-Viewer](https://github.com/spicecat/multisc-viewer/tree/main).
...
This package depends on `plumber2`, `Seurat` and other tidyverse/base packages; `pak` will resolve these automatically.
...
`msc_plumb()` reads the data root from an environment variable before starting and sets internal `options()` used by API handlers.
...
A Dockerfile is provided (see `MultiSCViewerR/Dockerfile`). Build and run:
...
- `...` – forwarded to `plumber2::api_run()` (e.g. `host`, `swagger = TRUE`)

Side-effects: sets options

- `MultiSCViewer.DATA_DIR`

This is derived from `MULTISC_VIEWER_DATA_DIR` (with defaults) and is a normalized path.

...
This package depends on `plumber`, `Seurat` and other tidyverse/base
...
`msc_plumb()` reads paths from environment variables before starting and
sets internal `options()` used by API handlers.
...
A Dockerfile is provided (see `multisc-daemon/Dockerfile`). Build and
run:
...
- `...` – forwarded to `plumber::plumb()$run()` (e.g. `host`,
  `swagger = TRUE`)
...
These are derived from corresponding environment variables (with
defaults) and are normalized paths.
